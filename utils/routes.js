	...
	appRoutes(){
		...
		this.app.post('/registerUser', async (request,response) => {
			...
			if(data.username === '') {
	            ...
	            registrationResponse.message = `Username can't be empty.`;
		...
		this.app.post('/login',async (request,response) =>{
			...
			if(data.username === '' || data.username === null) {
	            ...
	            loginResponse.message = `Username can't be empty.`;
	            response.status(412).json(loginResponse);
	        }else if(data.password === '' || data.password === null){				            
	            loginResponse.error = true;
	            loginResponse.message = `Password can't be empty.`;
	            response.status(412).json(loginResponse);
	        }else{
		...
		this.app.post('/getMessages',async (request,response) => {
			...
			if (userId === '') {
				messages.error = true;
	            messages.message = `User ID cant be empty.`;
