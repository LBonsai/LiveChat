...
class Helper{
	...
	async registerUser(params){
		try {
			return await this.db.query("INSERT INTO user (`firstname`,`lastname`,`username`,`password`,`online`) VALUES (?,?,?,?,?)", [params['firstname'], params['lastname'], params['username'], params['password'], 'N']);
		} catch (error) {
			console.error(error);
			return null;
		}
	...
	async loginUser(params){
		try {
			return await this.db.query(`SELECT id, firstname, lastname FROM user WHERE LOWER(username) = ? AND password = ?`, [params.username,params.password]);
		} catch (error) {
			return null;
		}
	}
	...
	async userSessionCheck(userId){
		try {
			const result = await this.db.query(`SELECT initcap(firstname) as firstname, initcap(lastname) as lastname, id, socketid, username, password, online FROM user WHERE id = ? AND online = ?`, [userId,'Y']);
			if(result !== null){
	...
	getChatList(userId, userSocketId){
		...
		try {
			return Promise.all([
				this.db.query(`SELECT initcap(firstname) as firstname, initcap(lastname) as lastname, id, socketid, username, password, online FROM user WHERE id = ?`, [userId]),
				this.db.query(`SELECT initcap(firstname) as firstname, initcap(lastname) as lastname, id, socketid, username, password, online FROM user WHERE online = ? and socketid != ?`, ['Y',userSocketId])
			]).then( (response) => {
				return {
